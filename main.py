...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning jarvis, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon jarvis, it's {day} and the time is {t}")
    else:
        speak(f"Good evening jarvis, it's {day} and the time is {t}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ('facebook' in query) or ('Youtube' in query) or ('whatsapp' in query) or ('instagram' in query):
            social_media(query)
